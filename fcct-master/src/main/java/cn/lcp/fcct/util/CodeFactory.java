...
public class CodeFactory {
    ...
    private static Boolean isCreateGetset;
    ...
    public static CodeFactory getInstance(){
        if(instance == null){
            ...
            try {
                ...
                isCreateGetset = Boolean.parseBoolean(properties.getProperty("IS_CREATE_GETSET").trim());
    ...
    public void CreateCode(String tableName,List<InformationSchema> informationSchemas){
        ...
        if(conversionTableName.startsWith("Tbl")){
            conversionTableName = conversionTableName.replace("Tbl", "");
        }
        ...
        try{
            for (int i = 0; i < files.size(); i++) {
                ...
                }else if("Dao.java".equals(codeTemplates.get(i))){
                    file = new File(files.get(i) + conversionTableName + "Mapper.java");
                }else if("Mapper.java".equals(codeTemplates.get(i))){
                    file = new File(files.get(i) + conversionTableName + "Mapper.xml");
                }else {
    ...
    private Map<String,String> getEntity(String tableName,List<InformationSchema> informationSchemas){
        ...
        for (int i = 0; i < informationSchemas.size(); i++) {
            ...
            if(!exclude_fileds.contains(informationSchemas.get(i).getColumnName())){
                poParamaterEntity.append("    //").append(informationSchemas.get(i).getColumnComment()+"\n")
